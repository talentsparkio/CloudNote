...
class NoteViewController: UIViewController, UINavigationControllerDelegate, UITextFieldDelegate, UIImagePickerControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        populateDateAsNecessary()

    }

    func populateDateAsNecessary() {
        if let note = note {
            let formatter = NSDateFormatter()
            formatter.dateStyle = NSDateFormatterStyle.ShortStyle
            formatter.timeStyle = .ShortStyle
            if let created = note.createdAt, updated = note.updatedAt {
                createdAt.text = "Created at: " + formatter.stringFromDate(created)
                updatedAt.text = "Updated at: " + formatter.stringFromDate(updated)
            }
        } else {
            createdAt.text = ""
            updatedAt.text = ""
